...
export const loginService = ({ phone, password }) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.User.findOne({ // findOne sẽ trả về 1 instance của sequelize nên sẽ không lấy được => raw: true
            where: { phone },
            raw: true // kết quả trả về dạng obj để có thể lấy được 
        })
        const isCorrectPassword = response && bcrypt.compareSync(password, response.password)// so sánh password client gửi lên với password trong db
